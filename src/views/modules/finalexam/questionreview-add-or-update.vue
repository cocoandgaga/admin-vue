    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/finalexam/questionreview/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.title = data.questionReview.title
                this.dataForm.description = data.questionReview.description
                this.dataForm.gmtCreate = data.questionReview.gmtCreate
                this.dataForm.gmtModified = data.questionReview.gmtModified
                this.dataForm.creator = data.questionReview.creator
                this.dataForm.viewCnt = data.questionReview.viewCnt
                this.dataForm.likeCnt = data.questionReview.likeCnt
                this.dataForm.commentCnt = data.questionReview.commentCnt
                this.dataForm.tag = data.questionReview.tag
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/finalexam/questionreview/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
