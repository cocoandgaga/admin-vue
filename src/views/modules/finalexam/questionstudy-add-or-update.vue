    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/finalexam/questionstudy/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.title = data.questionStudy.title
                this.dataForm.description = data.questionStudy.description
                this.dataForm.gmtCreate = data.questionStudy.gmtCreate
                this.dataForm.gmtModified = data.questionStudy.gmtModified
                this.dataForm.creator = data.questionStudy.creator
                this.dataForm.viewCnt = data.questionStudy.viewCnt
                this.dataForm.likeCnt = data.questionStudy.likeCnt
                this.dataForm.commentCnt = data.questionStudy.commentCnt
                this.dataForm.tag = data.questionStudy.tag
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/finalexam/questionstudy/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
